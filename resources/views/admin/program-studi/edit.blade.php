    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form action="{{ route('admin.program-studi.update', $programStudi) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="code" id="code" value="{{ old('code', $programStudi->code) }}"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $programStudi->name) }}"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500"
                    required>
                @error('name')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">{{ old('description', $programStudi->description) }}</textarea>
                @error('description')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
            </div>
