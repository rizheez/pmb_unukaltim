<x-admin-layout>
            ...
            @if($student->studentBiodata)
                <dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <dd class="mt-1 text-sm text-gray-900">{{ $student->studentBiodata->school_origin }} ({{ $student->studentBiodata->major }})</dd>
                    </div>
                </dl>
                <div class="mt-4">
                    <img src="{{ $student->studentBiodata->photo_url }}" alt="Student Photo" class="h-32 w-32 object-cover rounded-md border">
                </div>
            @else
            ...
            @if($student->registration)
                <dl class="grid grid-cols-1 gap-x-4 gap-y-4">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm text-gray-900">{{ $student->registration->registration_type }}</dd>
                    </div>
                    <div>
    ...
    @if($student->studentBiodata)
        <div class="mt-6 bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
            ...
            <form action="{{ route('admin.students.verify', $student->studentBiodata->id) }}" method="POST" class="space-y-6">
                @csrf
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            @if($student->studentBiodata->photo_path)
                                <img src="{{ $student->studentBiodata->photo_url }}" alt="Photo" class="h-32 w-32 object-cover rounded-md border mb-3">
                                @php
                                    $photoVerification = $student->studentBiodata->verifications->where('document_type', 'photo')->first();
                                @endphp
                                @if($photoVerification)
                                    <div class="mb-2">
                                        <span class="text-sm">Status: {!! $photoVerification->status_badge !!}</span>
                                        @if($photoVerification->notes)
                                            <p class="text-sm text-gray-600 mt-1">Catatan: {{ $photoVerification->notes }}</p>
                                        @endif
                                    </div>
                                @endif
                            @else
                        ...
                        @if($student->studentBiodata->photo_path)
                            <div class="ml-4 w-64">
                                ...
                                <textarea name="verifications[0][notes]" rows="2" placeholder="Catatan (opsional)" class="w-full rounded-md border-gray-300 text-sm"></textarea>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            @if($student->studentBiodata->kk_path)
                                <a href="{{ $student->studentBiodata->kk_url }}" target="_blank" class="text-blue-600 hover:underline text-sm">Lihat Dokumen</a>
                                @php
                                    $kkVerification = $student->studentBiodata->verifications->where('document_type', 'kk')->first();
                                @endphp
                                @if($kkVerification)
                                    <div class="mt-2">
                                        <span class="text-sm">Status: {!! $kkVerification->status_badge !!}</span>
                                        @if($kkVerification->notes)
                                            <p class="text-sm text-gray-600 mt-1">Catatan: {{ $kkVerification->notes }}</p>
                                        @endif
                                    </div>
                                @endif
                            @else
                        ...
                        @if($student->studentBiodata->kk_path)
                            <div class="ml-4 w-64">
                                ...
                                <textarea name="verifications[1][notes]" rows="2" placeholder="Catatan (opsional)" class="w-full rounded-md border-gray-300 text-sm"></textarea>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            @if($student->studentBiodata->ktp_path)
                                <a href="{{ $student->studentBiodata->ktp_url }}" target="_blank" class="text-blue-600 hover:underline text-sm">Lihat Dokumen</a>
                                @php
                                    $ktpVerification = $student->studentBiodata->verifications->where('document_type', 'ktp')->first();
                                @endphp
                                @if($ktpVerification)
                                    <div class="mt-2">
                                        <span class="text-sm">Status: {!! $ktpVerification->status_badge !!}</span>
                                        @if($ktpVerification->notes)
                                            <p class="text-sm text-gray-600 mt-1">Catatan: {{ $ktpVerification->notes }}</p>
                                        @endif
                                    </div>
                                @endif
                            @else
                        ...
                        @if($student->studentBiodata->ktp_path)
                            <div class="ml-4 w-64">
                                ...
                                <select name="verifications[2][status]" class="w-full rounded-md border-gray-300 mb-2">
                                    <option value="">-- Pilih Status --</option>
                                    <option value="approved">Setujui</option>
                                    <option value="rejected">Tolak</option>
                                </select>
                                <textarea name="verifications[2][notes]" rows="2" placeholder="Catatan (opsional)" class="w-full rounded-md border-gray-300 text-sm"></textarea>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            @if($student->studentBiodata->certificate_path)
                                <a href="{{ $student->studentBiodata->certificate_url }}" target="_blank" class="text-blue-600 hover:underline text-sm">Lihat Dokumen</a>
                                @php
                                    $certVerification = $student->studentBiodata->verifications->where('document_type', 'certificate')->first();
                                @endphp
                                @if($certVerification)
                                    <div class="mt-2">
                                        <span class="text-sm">Status: {!! $certVerification->status_badge !!}</span>
                                        @if($certVerification->notes)
                                            <p class="text-sm text-gray-600 mt-1">Catatan: {{ $certVerification->notes }}</p>
                                        @endif
                                    </div>
                                @endif
                            @else
                        ...
                        @if($student->studentBiodata->certificate_path)
                            <div class="ml-4 w-64">
                                ...
                                <select name="verifications[3][status]" class="w-full rounded-md border-gray-300 mb-2">
                                    <option value="">-- Pilih Status --</option>
                                    <option value="approved">Setujui</option>
                                    <option value="rejected">Tolak</option>
                                </select>
                                <textarea name="verifications[3][notes]" rows="2" placeholder="Catatan (opsional)" class="w-full rounded-md border-gray-300 text-sm"></textarea>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            ...
                            <div class="text-sm space-y-1">
                                ...
                                <p><span class="text-gray-600">Asal Sekolah:</span> {{ $student->studentBiodata->school_origin }}</p>
                            </div>
                            @php
                                $biodataVerification = $student->studentBiodata->verifications->where('document_type', 'biodata')->first();
                            @endphp
                            @if($biodataVerification)
                                <div class="mt-2">
                                    <span class="text-sm">Status: {!! $biodataVerification->status_badge !!}</span>
                                    @if($biodataVerification->notes)
                                        <p class="text-sm text-gray-600 mt-1">Catatan: {{ $biodataVerification->notes }}</p>
                                    @endif
                                </div>
                            @endif
                        </div>
                        <div class="ml-4 w-64">
                            ...
                            <textarea name="verifications[4][notes]" rows="2" placeholder="Catatan (opsional)" class="w-full rounded-md border-gray-300 text-sm"></textarea>
                        </div>
                    </div>
                </div>
